...
!function($) {
  ...
  Uploader.prototype._findBoredInstanceUrl = function(cb) {
    console.log('_findBoredInstanceUrl');
  ...
  Uploader.prototype._findResponsiveInstance = function(instances, index, cb) {
    console.log('_findResponsiveInstance');
    if (!instances[index]) {
  ...
  Uploader.prototype.start = function() {
    ...
    if (this._options.formData) {
      ...
      if (this._options.formData instanceof FormData) {
        ...
        console.log('options.formData instanceof FormData, appended assemblyParams, this._options.formData = ', this._options.formData);
      } else {
        console.log('_options.formData not instanceof FormData');
        var formData = new FormData(this.$form.get(0));
        formData.append("params", JSON.stringify(assemblyParams));
        console.log('assemblyParams appended to formData = ', JSON.stringify(assemblyParams));

        for (var i = 0; i < this._options.formData.length; i++) {
          var tupel = this._options.formData[i];
          console.log('tupel = ', tupel);
          formData.append(tupel[0], tupel[1], tupel[2]);
        }
        this._options.formData = formData;
      }
      console.log('new xmlhttprequest, open send');
      var f = new XMLHttpRequest();
      f.open("POST", url);
      console.log('this._options.formData = ', this._options.formData);
      f.send(this._options.formData);
    } else {
      console.log('using $uploadform (why?)');
  ...
  Uploader.prototype._poll = function(query) {
    ...
    $.jsonp({
      ...
      success           : function(assembly) {
        ...
        console.log('assembly from poll = ', assembly);
  ...
  Uploader.prototype.renderProgress = function(assembly, isAssemblyComplete, waitIsTrue) {
    console.log('renderProgress, assembly = ', assembly);
    if (!this._options.modal) {
      return;
    }
