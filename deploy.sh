...
EXISTING_TASK_ARN=`aws ecs list-tasks --region us-west-2 --cluster production-cluster --service-name production-service | jq -r '.taskArns[0]'`
aws ecs stop-task --region us-west-2 --cluster production-cluster --task "$EXISTING_TASK_ARN" > /dev/null

# Get the existing task definition JSON for the production-task-definition
# family.
aws ecs describe-task-definition --region us-west-2 --task-definition production-task-definition | \
...
NEW_TASK_DEFINITION_ARN=`aws ecs register-task-definition --region us-west-2 --family production-task-definition --cli-input-json file://production-task-definition.json | jq -r '.taskDefinition.taskDefinitionArn'`
...
aws ecs update-service --region us-west-2 --cluster production-cluster --service production-service --task-definition "$NEW_TASK_DEFINITION_ARN" > /dev/null
